...
int main(void)
{
  ...
  while (1)
  {
	  //Flashes Pin 7 of Port B which is the left most LED
	HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);


	// ADC to LCD; TODO: Read POT1 value and write to LCD
	//Retrieves ADC value from ADC
	uint32_t val = pollADC();
	char output[8];
	//converts integer to a string and stores in the char array
...
void EXTI0_1_IRQHandler(void)
{
	...
	curr_millis = HAL_GetTick(); //gets the current system time
	//debounce code

	if (curr_millis - prev_millis >= 100){
	//wait 100 milliseconds after the the last time the code was executed
	//this if statement ensures only one button press is processed every 100 seconds
	//ensuring that no mechanical noise is interpreted
		if (delay_t == 500){
...
void writeLCD(char *char_in){
    delay(3000);
	lcd_command(CLEAR);
	//Function from the LCD c library to print the string on the LCD
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	//Start the ADC conversion using the function from HAL which takes in the ADCs address
	HAL_ADC_Start(&hadc);
	//The ADC will wait indefinitely for the conversion to complete
...
uint32_t ADCtoCCR(uint32_t adc_val)
{
  // TODO: Calculate CCR val using an appropriate equation
	...
	//Duty cycle  = CCR/ARR
	//Duty Cycle = (CCR / (ARR))
	//And duty cycle is adc_val/ADC_MAX
	//Therefore CCR = (adc_val/ADC_MAX) * ARR
	//A max adc_val corresponds to a max ARRval therefore the PWM frequency depends
	//on received adc_val
